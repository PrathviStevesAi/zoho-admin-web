...
import { publicRegisterCustomerAction, verifyCustomerEmailAction } from "@/actions/customer.public.actions";
...
export function PublicCustomerRegistrationForm() {
  ...
  const handleVerifyEmail = async () => {
    ...
    if (res.success && res.data) {
      const errorMsg = "This email is already exists";
      ...
      clearError("email");
      toast.success("Verified! continue with form.");
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const res = await publicRegisterCustomerAction({
  ...
  return (
    <div className="w-full bg-white rounded-xl shadow-lg border border-slate-100 p-6 md:p-8 relative max-w-5xl mx-auto animate-in fade-in duration-300">
      <div className="mb-8">
      ...
      <form onSubmit={handleRegister} className="space-y-6" noValidate>
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-800 uppercase tracking-wider ml-1">Email Address <span className="text-red-500">*</span></label>
                <div className="flex gap-2 w-full">
                  <div className="relative flex-1">
                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-700 z-10 pointer-events-none" />
                    <Input
                      type="email"
                      ...
                      onChange={(e) => {
                        ...
                        if (isEmailAlreadyRegistered) setIsEmailAlreadyRegistered(false);
                        if (isEmailVerified) setIsEmailVerified(false);
                      }}
                      className={getInputClassName(errors.email || verifyEmailError || undefined, true)}
                    />
                  </div>
                  <button
                    type="button"
                    ...
                    className="cursor-pointer h-12 px-6 bg-[#0064cb] text-white rounded-xl text-md font-semibold hover:bg-[#0052ae] disabled:opacity-50 transition-colors shrink-0 flex items-center justify-center min-w-[100px] shadow-sm"
                  >
                    {isVerifyingEmail ? (
                ...
                {verifyEmailError && !errors.email && <p className="text-red-500 text-[10px] mt-1 font-medium ml-1">{verifyEmailError}</p>}
              </div>
        ...
        <div className="flex justify-center mt-8">
          <Button
            type="submit"
            disabled={isRegistering || isEmailAlreadyRegistered || !isEmailVerified}
