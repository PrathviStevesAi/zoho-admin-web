...
import { clientFetchPreShiftCheckInAction } from "@/lib/client-actions";
...
export default function Precheck({ initialData, pagination }: { initialData: Record[]; pagination: Pagination }) {
  ...
  const {
    ...
    initialData,
    pagination,
    clientFetchPreShiftCheckInAction,
    500,
    dateFrom,
    dateTo
  );
    ...
    <Card className="w-full h-[510px] border-border rounded-sm bg-card shadow-sm flex flex-col gap-2">
      <CardHeader className="flex flex-row items-center justify-between px-7 py-2">
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          <ExportButton data={displayedData} columns={precheckTableColumns} fileName="Precheck_Shifts" />
        </div>
      </CardHeader>
