...
// Singleton for the service worker registration
...
async function getServiceWorkerRegistration() {
    ...
    if (typeof window !== "undefined" && "serviceWorker" in navigator) {
        try {
            // updateViaCache: 'none' forces the browser to fetch the SW file from the network
            // (not from HTTP cache), ensuring the latest version is always installed
            ...
            // Force check for a new version of the SW
...
// Singleton for the messaging instance
...
// --- Global listener management (HMR-safe) ---
if (typeof window !== "undefined") {
    ...
    // Track recently seen message IDs to deduplicate between onMessage and SW postMessage
...
function notifyListeners(payload, source) {
    // Deduplicate: create a key from the payload
...
function attachServiceWorkerBridge() {
    ...
    // Primary: BroadcastChannel (most reliable, works even before SW controls the page)
    try {
    ...
    // Fallback: postMessage from service worker
...
// Attach a focus-based refresh: when the user focuses back on the tab
// (e.g. after seeing a native browser notification), refresh notifications.
function attachFocusRefresh() {
    ...
    window.addEventListener("focus", () => {
        const now = Date.now();
        // Throttle: only refresh if at least 5 seconds since last focus-refresh
...
export const onMessageListener = (callback) => {
    ...
    // Attach the ServiceWorker bridge (always, as a fallback)
...
export const generateToken = async () => {
    try {
        ...
        if (permission === "granted") {
            ...
            const tokenOptions = {
                vapidKey: "BB_rSefNooeDgWCh_sXZZR1ormy6cg2zmL6klyCNCQSP-XDmYGa9FYn1mpkILVmTxP5l9VRDWXhSQxPV1zwwnug",
            };
            if (swReg) {
