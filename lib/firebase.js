...
export const onMessageListener = (callback) => {
    if (callback) {
        ...
        console.log(`[Firebase] New listener added. Total listeners: ${messageListeners.length}`);
    }
    ...
    const attach = async () => {
        ...
        if (window._fcmListenerAttached) {
            console.log("[Firebase] FCM listener already attached to messaging instance.");
            return;
        }
        ...
        if (messagingInstance) {
            ...
            onMessage(messagingInstance, (payload) => {
                ...
                console.log(`[Firebase] Notifying ${messageListeners.length} listeners...`);
                messageListeners.forEach((cb, index) => {
                    try {
                        cb(payload);
                    } catch (err) {
            ...
            window._fcmListenerAttached = false; // Allow retry if it failed
        }
    };
    attach();
