...
function notifyListeners(payload, source) {
    ...
    const body = payload?.notification?.body || payload?.data?.body || payload?.notification?.message || payload?.data?.message || payload?.message || "";
    
    // Create a robust dedupe key based on content to prevent duplicates from different message sources
    const contentKey = title || body ? `${title}-${body}` : null;
    const fallbackId = payload?.messageId || payload?.fcmMessageId || payload?.data?.notification_id || String(Date.now());
    const dedupeKey = contentKey || fallbackId;
    ...
    setTimeout(() => window._fcmSeenMessages.delete(dedupeKey), 5000);
...
function attachServiceWorkerBridge() {
    ...
    if ("serviceWorker" in navigator) {
        ...
        console.log("[Firebase] ServiceWorker not supported, skipping message bridge.");
    }
}
...
export const generateToken = async () => {
    try {
        ...
        if (permission === "granted") {
            ...
            const tokenOptions = {
                vapidKey: process.env.NEXT_PUBLIC_VAPID_KEY || process.env.vapidKey || "BB_rSefNooeDgWCh_sXZZR1ormy6cg2zmL6klyCNCQSP-XDmYGa9FYn1mpkILVmTxP5l9VRDWXhSQxPV1zwwnug"
            };
            if (swReg) {
        ...
        console.warn("Detailed error in generateToken (expected if offline or blocked by adblocker):", error);
